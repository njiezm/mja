            ...
            <div class="p-8">
                <div class="flex items-center gap-3 mb-1">
                    <div class="w-10 h-10 bg-mja-blue/10 rounded-xl flex items-center justify-center">
                        <i class="fas fa-key text-mja-blue"></i>
                    </div>
                    <h1 class="font-display font-black text-xl text-gray-900">Nouveau mot de passe</h1>
                </div>
                <p class="text-gray-500 text-sm mb-7">Choisissez un nouveau mot de passe sécurisé.</p>
                ...
                <div class="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 mb-6 text-sm flex items-center gap-2 font-display font-semibold">
                    <i class="fas fa-exclamation-circle text-red-500 shrink-0"></i> {{ $errors->first() }}
                </div>
                @endif
                ...
                <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-display font-bold text-gray-700 mb-1.5">
                            <i class="fas fa-lock text-mja-blue mr-1"></i> Nouveau mot de passe
                        </label>
                        <div class="relative">
                            <input type="password" name="password" id="pw" required minlength="8"
                                   class="w-full border-2 border-gray-100 focus:border-mja-blue rounded-xl px-4 py-3 pr-12 text-sm outline-none transition-colors"
                                   placeholder="Minimum 8 caractères">
                            <button type="button" onclick="togglePw('pw','eye1')" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
                    ...
                    <div>
                        <label class="block text-sm font-display font-bold text-gray-700 mb-1.5">
                            <i class="fas fa-lock text-mja-blue mr-1"></i> Confirmer le mot de passe
                        </label>
                        <div class="relative">
                            <input type="password" name="password_confirmation" id="pw2" required minlength="8"
                                   class="w-full border-2 border-gray-100 focus:border-mja-blue rounded-xl px-4 py-3 pr-12 text-sm outline-none transition-colors">
                            <button type="button" onclick="togglePw('pw2','eye2')" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
